...
export async function getWeekly(params: { from: string; to: string; scope: VisibilityScope }) {
  ...
  const res = await fetch(`/api/reports/weekly?${qs}`, { headers: { ...authHeader() } })
...
export async function postWeeklyExport(params: { from: string; to: string; scope: VisibilityScope }) {
  const qs = new URLSearchParams(params as any)
  const res = await fetch(`/api/reports/weekly/export?${qs}`, { method: 'POST', headers: { ...authHeader() } })
  const j = await res.json()
...
export async function getExportStatus(jobId: string) {
  const res = await fetch(`/api/reports/weekly/export/status?id=${encodeURIComponent(jobId)}`, { headers: { ...authHeader() } })
  const j = await res.json()
...
export function getExportDownloadUrl(jobId: string): string {
  return `/api/reports/weekly/export/download?id=${encodeURIComponent(jobId)}`
}
